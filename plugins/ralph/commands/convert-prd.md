...
allowed-tools: Read, Write, Glob, Bash(mkdir:*, mv:*, git:*)
---
...
First, check for PRD files:
- Look in `tasks/` directory for `prd-*.md` files
- If none found, tell user: "No PRD files found. Use `/prd` first to generate one."
- If multiple found, ask which one to convert
...
2. Create/clear `progress.txt` with header:
   ...
   Learnings and patterns discovered during implementation.
