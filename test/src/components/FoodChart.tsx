...
export function FoodChart({ metrics }: FoodChartProps) {
  ...
  const avgFood =
    chartData.length > 0
      ? chartData.reduce((sum, d) => sum + d.food, 0) / chartData.length
      : 0;
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-md p-6">
      ...
      {chartData.length > 0 ? (
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-center py-8">
          No food data available for this time range
        </p>
      )}

      <div className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
        Average: {avgFood.toFixed(0)}g per day
      </div>
    </div>
  );
}
